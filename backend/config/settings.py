...
if not ENCRYPTION_KEY:
    if DEBUG:
        import warnings
        ENCRYPTION_KEY = hashlib.sha256(secrets.token_hex(32).encode()).hexdigest()
        warnings.warn(f'ENCRYPTION_KEY not set! Using a randomly generated key for this session. Set a persistent ENCRYPTION_KEY in .env for production.')
    else:
