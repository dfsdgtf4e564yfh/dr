...
SECRET_KEY = os.environ.get(
    'DJANGO_SECRET_KEY',
    'django-insecure-ckz5$7a^#@!' if DEBUG else None
)
if not SECRET_KEY:
...
FILE_UPLOAD_MAX_MEMORY_SIZE = int(os.environ.get('FILE_UPLOAD_MAX_MEMORY_SIZE', 500 * 1024 * 1024))
...
if not ENCRYPTION_KEY:
    import warnings
    warnings.warn('ENCRYPTION_KEY not set! Using derived key from SECRET_KEY (INSECURE). Set a separate ENCRYPTION_KEY in .env')
    ENCRYPTION_KEY = hashlib.sha256(SECRET_KEY.encode()).hexdigest() if SECRET_KEY else 'default-dev-key-32-bytes-long!!'
