...
def _r(text):
    """Reshape and reorder Persian text for proper PDF rendering"""
    t = str(text)
    try:
...
class ReportPDF:
    ...
    def _header_footer(self, canvas, doc):
        ...
        canvas.drawRightString(PAGE_WIDTH - MARGIN, 1 * cm,
                               _r(f'صفحه {doc.page}'))
        ...
        canvas.drawRightString(
            ...
            _r(f'تاریخ تولید: {jdatetime.datetime.now().strftime("%Y-%m-%d %H:%M")}')
        )
        canvas.restoreState()
    ...
    def add_bar_chart(self, data, labels, title='', width=480, height=200):
        ...
        chart.data = [[float(v) for v in data]]
        chart.categoryAxis.categoryNames = [_r(l) for l in labels]
        chart.categoryAxis.labels.fontName = 'Vazir'
        chart.categoryAxis.labels.fontSize = 7
        chart.valueAxis.valueMin = 0
        chart.valueAxis.valueMax = float(max(data)) * 1.2 if data else 1
    ...
    def add_doctor_signature(self, doctor_name=''):
        ...
        data = [
            ...
            [Paragraph(_r(doctor_name), styles['value']),
             Paragraph(_r(jdatetime.datetime.now().strftime('%Y/%m/%d')), styles['value'])],
        ]
