...
def full_backup_download(request):
    ...
    for mc in meta_candidates:
        if os.path.exists(mc):
            meta_path = mc
            break
    # Track temp files created for decrypted env so we can clean up
    tmp_env_files = []
    ...
    if meta_path:
        ...
        try:
            ...
            for ef in meta.get('extra_files', []):
                ...
                if os.path.exists(ef_path):
                    # Decrypt .env.enc files so user gets a usable .env in the ZIP
                    if ef_path.endswith('.env.enc'):
                        try:
                            from utils.encryption import get_fernet
                            fernet = get_fernet()
                            with open(ef_path, 'rb') as ef_fp:
                                encrypted_data = ef_fp.read()
                            decrypted_data = fernet.decrypt(encrypted_data)
                            env_tmp = tempfile.NamedTemporaryFile(delete=False, suffix='.env')
                            env_tmp.write(decrypted_data)
                            env_tmp.close()
                            tmp_env_files.append(env_tmp.name)
                            files_to_include.append((env_tmp.name, os.path.basename(ef_path).replace('.enc', '')))
                            continue
                        except Exception as decrypt_e:
                            logger.warning(f'Could not decrypt env file {ef_path}: {decrypt_e}')
                            # Fall through to include encrypted version as-is
        ...
        def cleanup():
            ...
            if tmp_decrypted:
                try:
                    ...
                    pass
            for env_tmp_path in tmp_env_files:
                try:
                    if os.path.exists(env_tmp_path):
    ...
    try:
        ...
        if is_sqlite:
            ...
            try:
                ...
                # After DB restore, restore media and env BEFORE cleanup
                restore_results = {}
                if tmp_dir:
                    media_zips = []
                    ...
                    for root, _dirs, files in os.walk(tmp_dir):
                        for f in files:
                            ...
                            if (basename_lower.endswith('.env') and not basename_lower.endswith('.meta.json')) or basename_lower.endswith('.env.enc'):
                                env_files.append(os.path.join(root, f))
                    if env_files:
                        env_path = env_files[0]
                        try:
                            # If the env file is encrypted, decrypt it first
                            if env_path.endswith('.env.enc'):
                                from utils.encryption import get_fernet
                                try:
                                    fernet = get_fernet()
                                    ...
                                    with open(dest_env, 'w', encoding='utf-8') as df:
                                        df.write(env_content)
                                    restore_results['env'] = 'بازیابی شد (رمزگشایی شده)'
                                except Exception as decrypt_err:
                                    # If decryption fails (e.g. different key), copy as-is
                                    dest_env = os.path.join(settings.BASE_DIR, '.env')
                                    shutil.copy2(env_path, dest_env)
                                    restore_results['env'] = f'کپی شد (رمزگشایی ممکن نبود: {str(decrypt_err)[:80]})'
                            else:
                                dest_env = os.path.join(settings.BASE_DIR, '.env')
                                shutil.copy2(env_path, dest_env)
            ...
            try:
                ...
                # Restore media and env BEFORE cleanup
                restore_results = {}
                if tmp_dir:
                    media_zips = []
                    for root, _dirs, files in os.walk(tmp_dir):
