...
from .tasks import execute_backup, _safe_sqlite_copy, _get_checksum, _verify_backup
...
def full_backup_download(request):
    ...
    if is_encrypted:
        try:
            ...
            return Response({'error': f'خطا در رمزگشایی بک‌آپ: {str(e)[:200]}. کلید رمزنگاری可能 متفاوت است.'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    else:
        ...
        if is_zip:
            ...
            with zipfile.ZipFile(zip_path, 'r') as zf:
                ...
                inner_files = [
                    ...
                    or f.endswith('.json') or f.endswith('.enc')
                ]
                if not inner_files:
                    shutil.rmtree(tmp_dir)
                ...
                # Check for .enc files (encrypted backups)
                enc_files = [f for f in inner_files if f.endswith('.enc')]
                if enc_files:
                    inner_path = os.path.join(tmp_dir, enc_files[0])
                    # Try to decrypt
                    from utils.encryption import get_fernet
                    try:
                        fernet = get_fernet()
                        ...
                        shutil.rmtree(tmp_dir)
                        return Response({'error': f'خطا در رمزگشایی فایل پشتیبان: {str(decrypt_err)[:200]}. کلید رمزنگاری سرور可能 متفاوت است.'}, status=status.HTTP_400_BAD_REQUEST)
                else:
                    inner_path = os.path.join(tmp_dir, inner_files[0])
                    is_sqlite = inner_path.endswith('.db') or inner_path.endswith('.sqlite') or inner_path.endswith('.sqlite3')
                tmp_path = inner_path
        elif is_enc:
            ...
            try:
                ...
                shutil.rmtree(tmp_dir)
                return Response({'error': f'خطا در رمزگشایی: {str(decrypt_err)[:200]}. کلید رمزنگاری可能 متفاوت است.'}, status=status.HTTP_400_BAD_REQUEST)
        else:
...
def github_callback(request):
    ...
    # Find the user from state param
    from django.contrib.auth import get_user_model
    User = get_user_model()
    try:
        user_id = int(request.GET.get('state', 0))
