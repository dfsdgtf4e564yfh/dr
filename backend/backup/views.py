    ...
    try:
        ...
        if is_sqlite:
            ...
            try:
                ...
                restore_results = {}
                if tmp_dir:
                    media_zips = [f for f in os.listdir(tmp_dir) if f.endswith('_media.zip')]
                    if media_zips:
                        media_zip_path = os.path.join(tmp_dir, media_zips[0])
                        try:
                    ...
                    env_files = [f for f in os.listdir(tmp_dir) if f.endswith('.env') and not f.endswith('.meta.json')]
                    if env_files:
                        env_path = os.path.join(tmp_dir, env_files[0])
                        try:
