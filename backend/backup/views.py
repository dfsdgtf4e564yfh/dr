...
def full_backup_download(request):
    ...
    # execute_backup already verified the DB before bundling into ZIP.
    # If encrypted, decrypt to a temp file and serve that.
    if raw_path.endswith('.enc'):
        try:
            ...
            tmp_decrypted = tempfile.NamedTemporaryFile(delete=False, suffix='.zip')
            tmp_decrypted.write(decrypted_data)
            tmp_decrypted.close()
            download_path = tmp_decrypted.name
            download_filename = os.path.basename(raw_path)[:-4]
        except Exception as e:
        ...
        response = FileResponse(open(download_path, 'rb'), as_attachment=True, filename=download_filename)
        ...
        def cleanup_decrypted():
            try:
                ...
                pass
        response._resource_closers.append(cleanup_decrypted)
        return response

    # Not encrypted — serve the ZIP directly
    file_size = os.path.getsize(raw_path)
    response = FileResponse(open(raw_path, 'rb'), as_attachment=True, filename=os.path.basename(raw_path))
    response['Content-Length'] = file_size
    return response
