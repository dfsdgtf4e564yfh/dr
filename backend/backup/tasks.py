...
def execute_backup(backup_type: str = 'auto') -> dict:
    ...
    jalali_ts = jnow.strftime('%Y%m%d_%H%M%S')
    ...
    try:
        if 'sqlite' in engine:
            ...
            # Populate raw fields from encrypted ones so they survive key changes
            try:
                call_command('backfill_raw_fields')
            except Exception as be:
                logger.warning(f'backfill_raw_fields before dumpdata: {be}')
            out = StringIO()
        ...
        zip_path = os.path.join(backup_dir, f'clinic_backup_{timestamp}.zip')
        with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zf:
            zf.write(output, os.path.basename(output))
            for ef in extra_files:
                ef_path = os.path.join(backup_dir, ef.get('file', ''))
                if os.path.exists(ef_path):
                    zf.write(ef_path, os.path.basename(ef_path))
                    os.unlink(ef_path)
        os.unlink(output)
        output = zip_path
        extra_files.clear()
