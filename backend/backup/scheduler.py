...
def get_schedule_config():
    ...
    obj, _ = BackupSchedule.objects.get_or_create(id=1, defaults={
        'enabled': True, 'hour': 3, 'minute': 0, 'day': 1, 'retention_days': 60,
    })
    return obj


def update_schedule(hour, minute, enabled, day=None):
    from backup.models import BackupSchedule
    ...
    if day is not None:
        obj.day = day
    obj.save()
    start_scheduler(hour, minute, getattr(obj, 'day', 1))
    return obj


def start_scheduler(hour=None, minute=None, day=None):
    if hour is None or minute is None:
        cfg = get_schedule_config()
        hour, minute = cfg.hour, cfg.minute
        day = cfg.day if hasattr(cfg, 'day') else 1
    if scheduler.get_job(SCHEDULER_JOB_ID):
        scheduler.remove_job(SCHEDULER_JOB_ID)
    # Run on the specified day of each month (default: 1st) at the given time
    scheduler.add_job(
        run_backup,
        CronTrigger(day=day, hour=hour, minute=minute, timezone='Asia/Tehran'),
        id=SCHEDULER_JOB_ID,
        replace_existing=True,
    )
    if not scheduler.running:
        scheduler.start()
    logger.info(f'Scheduler started: monthly backup on day {day} at {hour:02d}:{minute:02d}')
...
def init_scheduler():
    ...
    # Support both manage.py runserver (RUN_MAIN) and daphne/gunicorn (SERVER_STARTED)
    if os.environ.get('RUN_MAIN') == 'true' or os.environ.get('SERVER_STARTED') or os.environ.get('DJANGO_SERVER_TYPE'):
        _do_init_scheduler()
    else:
...
def _do_init_scheduler():
    try:
        cfg = get_schedule_config()
        if cfg.enabled:
            day = getattr(cfg, 'day', 1)
            start_scheduler(cfg.hour, cfg.minute, day)
