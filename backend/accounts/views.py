...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def reset_totp(self, request, pk=None):
        if request.user.role != 'admin':
            return Response({'error': 'فقط مدیر کلینیک می‌تواند تایید دو مرحله‌ای را ریست کند'}, status=status.HTTP_403_FORBIDDEN)
        ...
        AuditLog.objects.create(
            ...
            model_name='User',
            object_id=target.id,
            details={'username': target.username, 'reset_by': request.user.username}
        )
        return Response({'message': f'Google Authenticator کاربر {target.get_full_name()} با موفقیت ریست شد.'})
