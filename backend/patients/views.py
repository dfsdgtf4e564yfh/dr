...
class PatientViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role == 'admin':
            ...
            qs = qs.filter(
                db_models.Q(created_by__role='rtms') | db_models.Q(visible_to_rtms=True)
            ).distinct()
    ...
    def perform_create(self, serializer):
        ...
        if self.request.user.role == 'rtms':
            extra['visible_to_rtms'] = True
        patient = serializer.save(**extra)
    ...
    def all_deleted(self, request):
        ...
        if user.role == 'admin':
            ...
            appointments_qs = appointments_qs.filter(created_by__role='rtms')
        ...
        if user.role == 'admin':
            ...
            records_qs = records_qs.filter(created_by__role='rtms')
        ...
        if user.role == 'admin':
            ...
            billings_qs = billings_qs.filter(created_by__role='rtms')
    ...
    def search(self, request):
        ...
        if user.role != 'admin':
            if user.is_doctor_like:
                patient_ids = Appointment.objects.filter(doctor=user, status='completed').values_list('patient_id', flat=True).distinct()
                qs = qs.filter(id__in=patient_ids)
            elif user.role == 'rtms':
                qs = qs.filter(
                    db_models.Q(created_by__role='rtms') | db_models.Q(visible_to_rtms=True)
        ...
        if field == 'national_id':
            ...
            if q.isdigit() and len(q) == 10:
                conditions.append(db_models.Q(national_id_hash=hash_value(q)))
            elif q.isdigit() and len(q) == 11 and q.startswith('09'):
                conditions.append(db_models.Q(phone_hash=hash_value(q)))
    ...
    @action(detail=True, methods=['post'])
    def toggle_rtms_visibility(self, request, pk=None):
        if request.user.role not in ('admin', 'reception'):
            return Response({'error': 'شما اجازه این عملیات را ندارید'}, status=status.HTTP_403_FORBIDDEN)
        patient = self.get_object()
        patient.visible_to_rtms = not patient.visible_to_rtms
        patient.save(update_fields=['visible_to_rtms'])
        return Response({'visible_to_rtms': patient.visible_to_rtms})
            ...
            for idx, row in enumerate(rows[1:], start=2):
                try:
                    ...
                    Patient.objects.create(
                        ...
                        visible_to_rtms=request.user.role == 'rtms',
                    )
                    imported += 1
...
class SupportMessageViewSet(viewsets.ModelViewSet):
    ...
    def reply(self, request, pk=None):
        if not request.user.has_perm('support_message_reply'):
