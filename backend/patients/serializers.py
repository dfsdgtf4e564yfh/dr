...
from .city_codes import is_valid_city_code, get_city_from_national_id
...
class PatientListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'file_number', 'first_name', 'last_name', 'gender', 'father_name', 'national_id',
                  ...
                  'medical_history', 'created_at', 'created_by', 'education', 'job', 'old_file_number',
                  'tags']
...
class PatientDetailSerializer(serializers.ModelSerializer):
    ...
    def validate_national_id(self, value):
        ...
        if not is_valid_city_code(value):
            raise serializers.ValidationError('سه رقم اول کد ملی مربوط به هیچ شهری نیست')
        if Patient.objects.filter(national_id=value).exclude(
...
class ReferralLetterSerializer(serializers.ModelSerializer):
    ...
    def get_from_doctor_signature(self, obj):
        if obj.from_doctor and obj.from_doctor.signature:
            return obj.from_doctor.signature.url
        return ''
