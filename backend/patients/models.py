...
class Patient(BaseModel):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.national_id_raw and not self.national_id_enc:
            self._national_id = self.national_id_raw
            self.national_id_raw = ''
        if self.phone_raw and not self.phone_enc:
            self._phone = self.phone_raw
            self.phone_raw = ''
        if self.emergency_phone_raw and not self.emergency_phone_enc:
            self._emergency_phone = self.emergency_phone_raw
            self.emergency_phone_raw = ''
        if self.address_raw and not self.address_enc:
            self._address = self.address_raw
            self.address_raw = ''
        if self.medical_history_raw and not self.medical_history_enc:
