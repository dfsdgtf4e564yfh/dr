...
from utils.encryption import EncryptedField, hash_value
...
class Patient(BaseModel):
    ...
    visible_to_rtms = models.BooleanField(default=False, verbose_name='قابل مشاهده برای کاربران ویژه')
    ...
    def national_id(self, value):
        ...
        self.national_id_raw = ''
        self.national_id_hash = hash_value(value or '')
    ...
    def phone(self, value):
        self._phone = value
        self.phone_raw = ''
        self.phone_hash = hash_value(value or '')
...
def _patient_post_save_clear_raw(sender, instance, created, raw, **kwargs):
    if raw and created:
        needs_update = {}
        if instance.national_id_enc and instance._national_id:
            needs_update['national_id_raw'] = ''
        if instance.phone_enc and instance._phone:
            needs_update['phone_raw'] = ''
        if instance.emergency_phone_enc and instance._emergency_phone:
            needs_update['emergency_phone_raw'] = ''
        if instance.address_enc and instance._address:
            needs_update['address_raw'] = ''
        if instance.medical_history_enc and instance._medical_history:
            needs_update['medical_history_raw'] = ''
        if needs_update:
