...
class Patient(BaseModel):
    ...
    def _get_encrypted(self, enc_field):
        try:
            return getattr(self, enc_field)
        except ValueError:
            return None

    @property
    def national_id(self):
        val = self._get_encrypted('_national_id')
        if val:
            return val
    ...
    @property
    def phone(self):
        val = self._get_encrypted('_phone')
        if val:
            return val
    ...
    def emergency_phone(self):
        val = self._get_encrypted('_emergency_phone')
        if val:
            return val
    ...
    @property
    def address(self):
        val = self._get_encrypted('_address')
        if val:
            return val
    ...
    def medical_history(self):
        val = self._get_encrypted('_medical_history')
        if val:
            return val
    ...
    def save(self, *args, **kwargs):
        ...
        self._sync_raw_fields()
        super().save(*args, **kwargs)

    def _sync_raw_fields(self):
        if self.national_id_enc:
            self.national_id_raw = self._get_encrypted('_national_id') or ''
        elif self.national_id_raw:
            self._national_id = self.national_id_raw
        if self.phone_enc:
            self.phone_raw = self._get_encrypted('_phone') or ''
        elif self.phone_raw:
            self._phone = self.phone_raw
        if self.emergency_phone_enc:
            self.emergency_phone_raw = self._get_encrypted('_emergency_phone') or ''
        elif self.emergency_phone_raw:
            self._emergency_phone = self.emergency_phone_raw
        if self.address_enc:
            self.address_raw = self._get_encrypted('_address') or ''
        elif self.address_raw:
            self._address = self.address_raw
        if self.medical_history_enc:
            self.medical_history_raw = self._get_encrypted('_medical_history') or ''
        elif self.medical_history_raw:
...
def _patient_post_save_backfill_raw(sender, instance, created, raw, **kwargs):
    if raw and created:
        needs_update = {}
        if instance.national_id_enc and not instance.national_id_raw:
            needs_update['national_id_raw'] = instance._get_encrypted('_national_id') or ''
        if instance.phone_enc and not instance.phone_raw:
            needs_update['phone_raw'] = instance._get_encrypted('_phone') or ''
        if instance.emergency_phone_enc and not instance.emergency_phone_raw:
            needs_update['emergency_phone_raw'] = instance._get_encrypted('_emergency_phone') or ''
        if instance.address_enc and not instance.address_raw:
            needs_update['address_raw'] = instance._get_encrypted('_address') or ''
        if instance.medical_history_enc and not instance.medical_history_raw:
            needs_update['medical_history_raw'] = instance._get_encrypted('_medical_history') or ''
        if needs_update:
            Patient.objects.filter(pk=instance.pk).update(**needs_update)
