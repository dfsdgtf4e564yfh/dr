...
class IsSmsAdmin(BasePermission):
    def has_permission(self, request, view):
        ...
        if request.user.role in ('admin', 'super_support', 'support'):
            return True
        sms_perms = {'sms_templates', 'sms_settings', 'sms_history', 'sms_bulk',
                     'messenger_settings', 'notifications', 'sms_credit', 'messenger_bale'}
        user_perms = set(request.user.page_permissions or [])
        return bool(sms_perms & user_perms)
...
def sms_settings(request):
    if request.method == 'GET':
        ...
        db_key = cs.sms_api_key or ''
        env_key = getattr(django_settings, 'SMS_API_KEY', '')
        has_key = bool(db_key or env_key)
        return Response({
            'sms_api_key': '***' if has_key else '',
            'has_sms_api_key': has_key,
