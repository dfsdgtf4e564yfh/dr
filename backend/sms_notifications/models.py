...
class MessengerSetting(models.Model):
    MESSENGER_CHOICES = [
        ('eitaa', 'ایتا'),
        ('bale', 'بله'),
        ('rubika', 'روبیکا'),
    ...
    messenger_type = models.CharField(
        max_length=20, choices=MESSENGER_CHOICES, unique=True, verbose_name='نوع پیام‌رسان',
        error_messages={'unique': 'این پیام‌رسان قبلاً ثبت شده است'}
    )
    name = models.CharField(max_length=50, verbose_name='نام')
    bot_token = models.CharField(max_length=255, blank=True, verbose_name='توکن ربات')
    chat_id = models.CharField(max_length=100, blank=True, verbose_name='شناسه گیرنده پیش‌فرض')
    ...
    def __str__(self):
        return f"{self.name} ({self.get_messenger_type_display()})"
...
class MessengerLog(models.Model):
    ...
    message_id = models.CharField(max_length=100, blank=True, verbose_name='شناسه پیام')
    error_message = models.TextField(blank=True, verbose_name='پیام خطا')
    created_at = models.DateTimeField(auto_now_add=True, verbose_name='تاریخ ارسال')
    ...
    def __str__(self):
        return f"{self.get_messenger_type_display()} - {self.recipient} - {self.get_status_display()}"
