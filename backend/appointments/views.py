...
class AppointmentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role == 'admin':
            ...
            qs = qs.filter(created_by__role='rtms')
    ...
    def perform_create(self, serializer):
        ...
        with transaction.atomic():
            last = Appointment.objects.select_for_update().filter(date=date_val).order_by('-daily_number').first()
    ...
    def patient_grouped(self, request):
        ...
        if user.role == 'admin':
            ...
            base_qs = base_qs.filter(created_by__role='rtms')
