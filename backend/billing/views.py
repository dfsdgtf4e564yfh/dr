...
class BillingViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        if self.action in ('restore', 'restore_all'):
            return [HasPermission('billing_restore')]
        if self.action in ('permanent_delete', 'permanent_delete_all'):
            return [HasPermission('billing_permanent_delete')]
        if self.action == 'deleted':
            return [HasPermission('billing_deleted_view')]
    ...
    def report(self, request):
        ...
        if user.is_doctor_like:
            ...
            billings = billings.filter(created_by__role='rtms')
