...
def _get_time_slots():
    ...
    if not slots:
        return list(DEFAULT_TIME_SLOTS)
    has_default = any(s in DEFAULT_TIME_SLOTS for s in slots)
    if has_default:
        return slots
    seen = set()
    merged = []
    for s in list(DEFAULT_TIME_SLOTS) + slots:
        if s not in seen:
            seen.add(s)
            merged.append(s)
    return merged
...
def _get_doctor():
    ...
    doctor = User.objects.filter(
        Q(first_name__icontains='محمد') & Q(last_name__icontains='طاهری'),
        is_active=True
    ).first()
    if doctor:
        return doctor
    doctor = User.objects.filter(
        Q(first_name__icontains='طاهری') | Q(last_name__icontains='طاهری'),
        is_active=True
    ).exclude(role__in=['reception', 'support', 'super_support']).first()
    if doctor:
        return doctor
...
def create_booking(request):
    ...
    with transaction.atomic():
        nhash = hash_value(data['national_id'])
        patient = Patient.objects.filter(national_id_hash=nhash, is_deleted=False).first()
        if not patient:
            ...
            for field in ['first_name', 'last_name', 'phone', 'gender', 'father_name', 'education', 'job', 'address', 'emergency_phone']:
                val = data.get(field)
                if val is not None and getattr(patient, field) != val:
                    setattr(patient, field, val)
            if data.get('birth_date') is not None and patient.birth_date != data['birth_date']:
    ...
    if payment_method == 'online':
        ...
        if merchant:
            ...
            return Response({'error': 'پرداخت آنلاین تنظیم نشده. لطفاً روش پرداخت دیگری انتخاب کنید.'}, status=400)
...
def clinic_settings(request):
    ...
    if request.method == 'GET':
        defaults = {
            'doctor_name': 'دکتر محمد طاهری',
            'specialty': 'متخصص اعصاب و روان (روانپزشک)',
            'tagline': 'درمان بدون دارو و بدون عوارض با rTMS، tDCS، CES',
            'address': 'بندرعباس، سه راه پلنگ صورتی، پشت بانک صادرات، کوچه دوم، جنب چاپخانه سپاهان (MRI جام جم سابق)',
            'phone': '07632229600',
            'phone2': '07632220252',
            'email': 'dr-mohammadtaheri@gmail.com',
            'instagram': 'https://www.instagram.com/dr_taheri_rtms',
            'google_maps': '',
            'waze': '',
            'balad': '',
            'neshan': '',
        ...
        configs = {c.key: c.value for c in ClinicConfig.objects.all()}
        defaults.update(configs)
        defaults['time_slots'] = ','.join(_get_time_slots())
