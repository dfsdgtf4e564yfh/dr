...
def clinic_info(request):
    info = {
        'doctor_name': get_config('doctor_name', 'دکتر محمد طاهری طاهرآباد'),
        ...
        'phone': get_config('phone', '076-32229600'),
        ...
        'zarinpal_merchant': get_config('zarinpal_merchant', ''),
    }
...
def patient_lookup(request):
    ...
    patient = Patient.objects.filter(national_id=national_id, is_deleted=False).first()
    if patient:
...
def create_booking(request):
    ...
    with transaction.atomic():
        patient = Patient.objects.filter(national_id=data['national_id'], is_deleted=False).first()
        if not patient:
...
def payment_callback(request):
    ...
    # Find billing by tracking billing.receipt_number stored in metadata would be ideal
    # Instead, search by appointment date/patient - simpler: find latest pending billing
