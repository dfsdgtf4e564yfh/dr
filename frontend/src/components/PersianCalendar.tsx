...
import { getPersianMonthData, getPersianMonthName, PERSIAN_MONTHS, WEEKDAYS, type PersianMonthData, type CalendarDay } from '../utils/persian_calendar'
...
export default function PersianCalendar({ year: propYear, month: propMonth, onDateSelect, onMonthChange, appointments, compact }: PersianCalendarProps) {
  ...
  const todayParts = today.toLocaleDateString('fa-IR').split('/')
  const [year, setYear] = useState(propYear || Number(todayParts[0]))
  const [month, setMonth] = useState(propMonth || Number(todayParts[1]))
  ...
  const goToday = () => {
    setYear(Number(todayParts[0])); setMonth(Number(todayParts[1]))
    onMonthChange?.(Number(todayParts[0]), Number(todayParts[1]))
  }

  const weekStart = 6 // Saturday in JS Sunday=0
  ...
  return (
    <div className={`bg-white rounded-2xl border border-surface-200 overflow-hidden ${compact ? '' : 'shadow-sm'}`}>
      <div className="flex items-center justify-between p-3 bg-surface-50/80 border-b border-surface-100">
        <button onClick={prevMonth} className="p-1.5 hover:bg-surface-200 rounded-xl transition-all"><ChevronRight size={18} /></button>
        ...
        <button onClick={nextMonth} className="p-1.5 hover:bg-surface-200 rounded-xl transition-all"><ChevronLeft size={18} /></button>
      </div>
                ...
                {day ? (
                  <button
                    onClick={() => {
                      if (onDateSelect) {
                        const g = new Date()
                        onDateSelect({ year: day.year, month: day.month, day: day.day, gregorian: g.toISOString() })
                      }
                    }}
