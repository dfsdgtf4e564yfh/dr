...
export default function SmsSendModal({ show, onClose, type, appointmentId, patientId, amount, onSuccess, billingAppointmentId }: SmsSendModalProps) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (type === 'payment') {
        res = await smsType.send(patientId, amount, msgText, null, isPatternMethod ? true : null, billingAppointmentId)
      } else {
        res = await smsType.send(appointmentId, msgText, null, null, isPatternMethod ? true : null)
      }
