...
const topItems: NavItem[] = [
  { to: '/dashboard', label: 'داشبورد', icon: LayoutDashboard, permission: 'dashboard' },
  { to: '/patients', label: 'بیماران', icon: Users, permission: 'patients' },
]
...
const groupColors: Record<string, { bg: string; text: string; border: string; dot: string }> = {
  ...
  'فرم‌ها': { bg: 'bg-rose-800/20', text: 'text-rose-300', border: 'border-rose-700/30', dot: 'bg-rose-400' },
}
...
const menuGroups: MenuGroup[] = [
  {
    ...
    items: [
      { to: '/appointments', label: 'نوبت‌ها', icon: Calendar, end: true, permission: 'appointments' },
      { to: '/appointments/calendar', label: 'تقویم نوبت‌ها', icon: Calendar, permission: 'appointments_calendar' },
      { to: '/waiting-list', label: 'لیست انتظار', icon: Clock, permission: 'waiting_list' },
      { to: '/appointments/report', label: 'گزارش نوبت‌ها', icon: FileBarChart, permission: 'appointments_report' },
      { to: '/calendar', label: 'تقویم شمسی', icon: Calendar, permission: 'calendar_view' },
    ],
  },
  {
    label: 'پزشکی', icon: FileText,
    items: [
      { to: '/medical-records', label: 'پرونده پزشکی', icon: FileText, permission: 'medical_records' },
      { to: '/doctor-finance', label: 'حساب پزشکان', icon: HandCoins, permission: 'doctor_finance' },
      { to: '/referral-letters', label: 'نامه‌های ارجاع', icon: Send, permission: 'referral_letters' },
      { to: '/diagnosis-report', label: 'گزارش تشخیص', icon: Activity, permission: 'diagnosis_report' },
      { to: '/diagnosis-drugs', label: 'تشخیص و دارو', icon: FileText, permission: 'diagnosis_drugs' },
      { to: '/visit-templates', label: 'قالب‌های ویزیت', icon: FileText, permission: 'medical_record_templates' },
      { to: '/dicom-viewer', label: 'تصاویر پزشکی', icon: Image, permission: 'dicom_viewer' },
    ],
  },
  {
    label: 'فرم‌ها', icon: FileText,
    items: [
      { to: '/tms-forms', label: 'فرم TMS', icon: Activity, permission: 'medical_records' },
    ],
  },
  {
    label: 'مالی', icon: DollarSign,
    items: [
      { to: '/billing', label: 'صورتحساب', icon: DollarSign, end: true, permission: 'billing' },
      { to: '/billing/report', label: 'گزارش مالی', icon: FileBarChart, permission: 'billing_report' },
      { to: '/settlements', label: 'تسویه حساب', icon: DollarSign, permission: 'settlements' },
    ],
  },
   {
     label: 'مدیریت سیستم', icon: Settings,
     items: [
       { to: '/notifications', label: 'اعلانات', icon: Bell, permission: 'notifications' },
      { to: '/users', label: 'کاربران', icon: UserCircle, permission: 'users' },
        { to: '/roles', label: 'نقش‌ها', icon: Shield, permission: 'roles_manage' },
        { to: '/backup', label: 'پشتیبان', icon: HardDrive, permission: 'backup' },
       { to: '/sms-templates', label: 'قالب‌های پیامک', icon: MessageSquare, permission: 'settings' },
       { to: '/sms-history', label: 'تاریخچه پیامک‌ها', icon: History, permission: 'settings' },
       { to: '/settings', label: 'تنظیمات', icon: Settings, permission: 'settings' },
       { to: '/deleted-items', label: 'موارد حذف شده', icon: Archive, permission: 'patients' },
      { to: '/activity-log', label: 'تاریخچه فعالیت‌ها', icon: Activity, permission: 'settings' },
     ],
   },
]
...
export default function Layout() {
  ...
  const groupPermMap: Record<string, string> = {
    ...
    'فرم‌ها': 'show_forms_menu',
  }
  const visibleTopItems = topItems.filter(item => hasPermission(item.permission!))
  const supportInboxItem = user?.role === 'support' || user?.role === 'rtms' ? { to: '/support-inbox', label: 'صندوق پیام‌ها', icon: Inbox, permission: 'support_inbox' } as NavItem : null
  if (supportInboxItem) visibleTopItems.push(supportInboxItem)
  const processedGroups = menuGroups
    .map(group => ({
      ...group,
      items: group.items.filter(item => hasPermission(item.permission!)),
    }))
    .filter(group => group.items.length > 0)
  const visibleGroups = processedGroups.filter(g => hasPermission(groupPermMap[g.label]))
  const flatItems = processedGroups
    .filter(g => !hasPermission(groupPermMap[g.label]))
    .flatMap(g => g.items.map(item => ({ ...item, icon: item.icon || g.icon } as NavItem)))
      ...
      <div className={`fixed inset-0 bg-black/20 backdrop-blur-sm z-40 lg:hidden ${sidebarOpen ? '' : 'hidden'}`}
        ...
        <div className="sidebar-footer">
          ...
          {isInstallable && !isInstalled && (
            <a href="#" onClick={(e) => { e.preventDefault(); install() }}
        ...
        <header className="topbar">
          ...
          <div className="topbar-left">
            ...
            <div ref={bellRef} className="relative">
              ...
              {notifOpen && (
                <div className="absolute left-0 top-full mt-2 w-72 bg-white border border-slate-200 rounded-xl shadow-lg z-50 overflow-hidden card-iranian">
            ...
            <div className="topbar-profile cursor-pointer" onClick={() => setProfileModal(true)}>
              ...
              <div className="topbar-avatar">
                {user?.avatar ? <img src={user.avatar} alt="" className="w-full h-full rounded-xl object-cover" /> : user?.first_name?.[0] || user?.username?.[0] || '?'}
              </div>
            </div>
          </div>
        </header>
