...
type MessengerType = 'eitaa' | 'bale' | 'rubika'

const MESSENGER_INFO: Record<MessengerType, { label: string; icon: string; color: string }> = {
  eitaa: { label: 'ایتا', icon: '💬', color: 'text-green-600' },
  bale: { label: 'بله', icon: '💬', color: 'text-blue-600' },
  rubika: { label: 'روبیکا', icon: '💬', color: 'text-purple-600' },
}
...
export default function MessengerSection() {
  ...
  const handleToggle = async (item: MessengerSetting) => {
    ...
    try {
      ...
      toast.success(`وضعیت ${MESSENGER_INFO[item.messenger_type as MessengerType].label} تغییر کرد`)
    } catch {
  ...
  const handleSave = async (item: MessengerSetting) => {
    ...
    try {
      ...
      toast.success(`تنظیمات ${MESSENGER_INFO[item.messenger_type as MessengerType].label} ذخیره شد`)
    } catch {
      toast.error('خطا در ذخیره تنظیمات')
    } finally {
      setSaving(null)
  ...
  const handleTest = async (item: MessengerSetting) => {
    ...
    try {
      ...
      if ((data as any).success) {
        toast.success(`اتصال ${MESSENGER_INFO[item.messenger_type as MessengerType].label} برقرار شد`)
      } else {
        toast.error((data as any).error || 'خطا در اتصال')
      }
    } catch {
      toast.error('عدم امکان اتصال')
    } finally {
      setTesting(null)
    }
  }
    ...
    <div className="panel card-iranian">
      ...
      <div className="panel-body space-y-4">
        ...
        {settings.length === 0 ? (
          ...
          settings.map((item) => {
            const info = MESSENGER_INFO[item.messenger_type as MessengerType]
            return (
