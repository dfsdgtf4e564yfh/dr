...
export default function SmsSection({ templates, setTemplates, setSmsSettingsModal }: SmsSectionProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      await updateSmsSettings({
        ...
        sms_api_base: smsApiBase,
        sms_line_number: smsLineNumber,
      } as any)
      toast.success('تنظیمات پیامک با موفقیت ذخیره شد')
    } catch (err: any) {
      const msg = err.response?.data?.error || err.response?.data?.detail || 'خطا در ذخیره تنظیمات پیامک'
      toast.error(msg)
    } finally {
      setSaving(false)
    }
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      const msg = err.response?.data?.error || err.response?.data?.detail || 'عدم امکان اتصال به سامانه ملی پیامک'
      toast.error(msg)
    } finally {
      setTesting(false)
    }
  ...
  const handleCredit = async () => {
    try {
      ...
      const msg = err.response?.data?.error || 'خطا در دریافت اعتبار'
      toast.error(msg)
    }
  }
        ...
        <div className="panel-body space-y-3">
          ...
          {templates.length === 0 ? (
            <div className="text-center py-6 text-slate-400">
              ...
              <p>هنوز الگویی تعریف نشده</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {templates.map(t => (
                <div key={t.id} className="border border-slate-200 rounded-xl p-4 space-y-2 hover:border-brand-300 transition-colors">
                  ...
                  <div className="flex items-center gap-3 text-xs text-slate-500">
                    ...
                    <span className="font-mono" dir="ltr">{(t as any).pattern_code}</span>
                  </div>
