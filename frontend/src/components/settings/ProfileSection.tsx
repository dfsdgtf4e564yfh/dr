          ...
          {diskHealth && (
            <div className="space-y-3 text-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {['DATA_DIR', 'STATIC_ROOT', 'MEDIA_ROOT'].map(key => {
                  const c = diskHealth.checks?.[key]
                  if (!c) return null
                  const du = diskHealth.checks?.disk_usage?.[key]
                  return (
                    <div key={key} className="bg-slate-50 rounded-lg p-3 border border-slate-200">
                      ...
                      {du && du.total && (
                        <p className="text-xs text-slate-400 mt-1">
                          {du.free} آزاد از {du.total} ({du.used_percent}% استفاده)
                        </p>
                      )}
                      {du && du.error && (
                        <p className="text-xs text-red-400 mt-1">خطا: {du.error}</p>
                      )}
                    </div>
                  )
                })}
