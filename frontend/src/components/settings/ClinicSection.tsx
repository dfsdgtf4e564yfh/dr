...
import { uploadSignature, clearSignature, getClinicSettings } from '../../services/api'
...
export default function ClinicSection({ clinic, setClinic, logoFile, setLogoFile, user, docProfile, setDocProfile }: ClinicSectionProps) {
  ...
  const saveDocProfile = async () => {
    try {
      const fd = new FormData()
      fd.append('specialization', docProfile.specialization)
      fd.append('medical_council_number', docProfile.medical_council_number)
      await uploadSignature(fd)
      toast.success('پروفایل پزشک با موفقیت ذخیره شد ')
    ...
    <>
            ...
            <div className="md:col-span-2">
              ...
              <div className="flex items-center gap-4 flex-wrap">
                <div className="w-20 h-20 border-2 border-dashed border-slate-300 rounded-xl flex items-center justify-center bg-slate-50 overflow-hidden">
                  ...
                  {!logoFile && clinic?.logo ? <img src={clinic.logo} alt="لوگو" className="max-w-full max-h-full object-contain p-1" /> : null}
      ...
      {user?.role === 'doctor' && (
        <>
          <div className="panel">
            <div className="panel-header">
              <h3 className="flex items-center gap-2"><User size={18} className="text-brand-500" /> پروفایل پزشک</h3>
            </div>
            <div className="panel-body space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
            ...
            <div className="panel-body">
              ...
              <div className="flex items-center gap-4 flex-wrap">
                <div className="w-32 h-20 border-2 border-dashed border-slate-300 rounded-lg flex items-center justify-center bg-slate-50 overflow-hidden">
                  {user?.signature ? (
                    <img src={user.signature} alt="امضا" className="max-w-full max-h-full object-contain p-1" />
