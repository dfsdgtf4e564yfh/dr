...
import {
  toJalaliNumeric, fromJalali, toPersianDigits,
...
export default function JalaliDateInput({ value, onChange, className = '', required, placeholder = 'انتخاب تاریخ', label, error }: JalaliDateInputProps) {
  ...
  useEffect(() => {
    if (isOpen && containerRef.current) {
      ...
      setPopupStyle({ position: 'fixed', zIndex: '2147483647', top: `${top}px`, right: `${right}px` })
    }
  }, [isOpen])
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      <button
        ...
        className={`
          input-field flex items-center justify-between gap-2
          ${error ? 'error' : ''}
          ${isOpen ? '!border-brand-500 ring-4 ring-brand-500/10' : ''}
          ${displayValue ? 'text-surface-800' : 'text-surface-400'}
          ${className}
        `}
      >
        <span>{displayValue || placeholder}</span>
        <CalendarDays size={18} className="text-surface-400 flex-shrink-0" />
      </button>
