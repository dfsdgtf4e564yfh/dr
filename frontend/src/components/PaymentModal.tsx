...
import { createBilling, updateBilling, payBalance, getBillings, requestOnlinePayment } from '../services/api'
  ...
  return (
    <Modal open={open} onClose={onClose} size="sm" title={isBalance ? 'پرداخت مانده حساب' : 'ثبت پرداخت'}>
      <div className="bg-slate-50 rounded-xl p-4 mb-4 space-y-2">
        {isBalance ? (
          ...
          <>
            ...
            <div className="flex justify-between text-sm"><span className="text-slate-500">پزشک / درمانگر:</span><span className="font-bold text-slate-800">{appointment.doctor_name}</span></div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {billingId && (
          <Button type="button" variant="outline" className="w-full" icon={ExternalLink}
            onClick={async () => {
              try {
                const { data } = await requestOnlinePayment(billingId)
