...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response?.status === 403) {
      error.message = 'شما دسترسی به این بخش ندارید'
    }
...
export const login = (username: string, password: string, otpCode?: string) => {
  if (otpCode) {
    return api.post<LoginResponse>('/auth/login/', { username, password, otp_step: 'verify', code: otpCode })
  }
...
export const setUserRestrictions = (id: number, restrictions: any) => api.patch(`/auth/users/${id}/set_restrictions/`, { restrictions })
...
export const resetUserTotp = (id: number) => api.post(`/auth/users/${id}/reset_totp/`)
...
export const togglePatientRtmsVisibility = (id: number) => api.post<{visible_to_rtms: boolean}>(`/patients/${id}/toggle_rtms_visibility/`)
...
export const createSupportMessage = (data: Partial<SupportMessage>) => {
  ...
  Object.entries(data).forEach(([k, v]: [string, unknown]) => {
    if (k === 'patient_name') return
    if (k === 'patient' && !v) return
...
export const verifyBackup = (filename: string) => api.post('/backup/verify/', { filename })
export const restoreMedia = (filename: string) => api.post(`/backup/restore-media/${encodeURIComponent(filename)}/`)
export const restoreEnvFile = (filename: string) => api.post(`/backup/restore-env/${encodeURIComponent(filename)}/`)
export const diskHealthCheck = () => api.get('/health/disk/')
...
export const updateGitHubConfig = (data: Record<string, any>) => api.put('/backup/github/config/', data)
...
// GitHub OAuth
export const getGitHubAuthUrl = () => api.get<{ url: string }>('/backup/github/auth-url/')
export const disconnectGitHub = () => api.post('/backup/github/disconnect/')
export const listGitHubRepos = () => api.get<GitHubReposResponse>('/backup/github/repos/')
export const getGitHubStatus = () => api.get<{ connected: boolean; github_user: string; repo: string }>('/backup/github/status/')
...
export const myAppointments = (nationalId: string, fileNumber?: string) =>
  api.get(`/online-booking/my-appointments/?national_id=${nationalId}${fileNumber ? `&file_number=${fileNumber}` : ''}`)
