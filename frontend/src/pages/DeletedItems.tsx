...
export default function DeletedItems() {
  ...
  const sections: DeletedSection[] = [
    ...
    { key: 'appointments', label: 'نوبت‌ها', icon: Calendar, permission: 'appointments',
      cols: ['بیمار', 'پزشک', 'تاریخ', 'ساعت', 'وضعیت', 'تاریخ حذف'],
      render: (item: any) => [
        item.patient_name || '—',
        item.doctor_name || '—',
    ...
    { key: 'medical_records', label: 'پرونده‌های درمانی', icon: FileText, permission: 'medical_records',
      cols: ['بیمار', 'پزشک', 'جلسه', 'تاریخ حذف'],
      render: (item: any) => [
        item.patient_name || '—',
        item.doctor_name || '—',
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button onClick={load} className="btn-secondary">بروزرسانی</button>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          {sections.map(section => {
            ...
            return (
              <div key={section.key} className="panel card-iranian">
                <div className="flex items-center justify-between mb-3 pb-2 border-b border-slate-100">
                  <div className="flex items-center gap-2">
                    <Icon size={18} className="text-brand-500" />
                    <h2 className="text-base font-bold text-slate-700">{section.label}</h2>
                    <span className="text-xs bg-red-100 text-red-600 px-2 py-0.5 rounded-full">{toPersianDigits(items.length)}</span>
                  </div>
                  <div className="flex items-center gap-2">
