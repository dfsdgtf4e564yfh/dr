  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-surface-800">حساب درمانگران</h1>
          <p className="text-sm text-surface-400 mt-1">مدیریت مالی و تسویه حساب درمانگران</p>
        </div>
      </div>
          ...
          <motion.div key={idx} variants={item} className="card p-5">
            <div className="flex items-center gap-3 mb-4 pb-3 border-b border-surface-100">
              ...
              <div>
                ...
                <span className="text-xs text-surface-400">درمانگر کلینیک</span>
              </div>
            </div>
      ...
      <motion.div variants={item} className="card p-6">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h3 className="font-bold text-surface-800">درآمد درمانگران (ماه جاری)</h3>
            <p className="text-xs text-surface-400">گزارش عملکرد مالی درمانگران</p>
          </div>
        </div>
          ...
          <div className="table-wrap">
            <table>
              <thead><tr>
                ...
                <th className="text-center">درآمد کل</th>
                <th className="text-center">دریافت شده</th>
                <th className="text-center">سهم پزشک / درمانگر</th>
              </tr></thead>
              <tbody>
                {report?.doctor_incomes?.length === 0 ? (
                  ...
                  <tr key={idx}>
                    <td className="text-center font-semibold text-surface-700">{d.doctor_name}</td>
                    <td className="text-center font-bold text-surface-800">{formatMoney(d.total)}</td>
                    <td className="text-center text-success-600 font-semibold">{formatMoney(d.paid)}</td>
                    <td className="text-center text-brand-600 font-bold">{formatMoney(d.share)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
