...
export default function ReferralLetters() {
  ...
  const selectPatient = (p: Patient) => {
    setForm(prev => ({
      ...prev,
      patient: p.id,
      description: internalMode ? `بیمار ${p.first_name} ${p.last_name}\n\n` : `همکار ارجمند باسلام و احترام\n\nبیمار ${p.gender === 'female' ? 'سرکار خانم' : 'جناب آقای'} ${p.first_name} ${p.last_name} با شماره پرونده ${p.file_number ? toPersianDigits(p.file_number) : '_________'} از تاریخ ${p.first_visit_date ? toJalali(p.first_visit_date) : '_________'} لغایت ${prev.date || '_________'} با نشانه‌ها و علائم\n\n\nجهت بررسی و اقدام درمانی لازم به حضورتان معرفی می‌گردد.`,
    }))
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  @media print { .print-btn { display: none; } body { background: #fff; padding: 0; } .print-page { box-shadow: none; padding: 20px 28px; } }
    ...
    const clinicFooter = clinicAddress || clinicPhone ? `<div class="print-footer">${clinicAddress ? `<div><span class="flbl">آدرس:</span><span class="fval"> ${clinicAddress}</span></div>` : ''}${clinicPhone ? `<div><span class="flbl">تلفن:</span><span class="fval"> ${toPersianDigits(clinicPhone)}${clinicPhone2 ? ` | ${toPersianDigits(clinicPhone2)}` : ''}</span></div>` : ''}</div>` : ''
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto shadow-xl">
            <div className="flex justify-between items-center mb-5">
              <h3 className="text-lg font-bold text-slate-800">ارجاع جدید</h3>
      ...
      {printData && (
        <div id="print-modal" className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-2xl shadow-xl overflow-y-auto max-h-[95vh]">
            <div className="sticky top-0 bg-white z-10 flex justify-between items-center px-6 py-4 border-b border-slate-200 rounded-t-2xl">
              <h3 className="text-lg font-bold text-slate-800">پیش‌نمایش نامه ارجاع</h3>
              <button onClick={() => setPrintData(null)} className="text-slate-400 hover:text-slate-600"><X size={20} /></button>
            </div>
            <div className="p-6" ref={printRef}>
              ...
              <div className="feedback-wrap mt-8 pt-5 border-t border-[#1e3a5f]">
                ...
                <textarea
                  ...
                  style={{ fontSize: 12, lineHeight: 2.2 }}
                />
            ...
            <div className="sticky bottom-0 bg-white border-t border-slate-200 px-6 py-4 flex gap-3 justify-center rounded-b-2xl">
