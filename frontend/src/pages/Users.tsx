...
import { getUsers, createUser, updateUser, deleteUser, getAvailablePermissions } from '../services/api'
...
const roleLabels: Record<string, string> = { admin: 'مدیر کلینیک', reception: 'پذیرش', doctor: 'پزشک / درمانگر', rtms: 'کاربر ویژه', support: 'پشتیبانی' }
...
const DEFAULT_ROLE_PERMISSIONS: Record<string, string[]> = {
  admin: ['dashboard', 'dashboard_income', 'patients', 'patient_info',
    ...
    'appointments', 'appointments_calendar', 'waiting_list', 'appointments_report',
    'medical_records', 'doctor_finance', 'referral_letters', 'diagnosis_report',
    'diagnosis_drugs', 'billing', 'billing_report', 'settlements',
    'notifications', 'users', 'backup', 'settings', 'support_inbox'],
  ...
  reception: ['dashboard', 'patients', 'patient_info', 'patient_appointments',
    ...
    'appointments', 'appointments_calendar', 'waiting_list', 'billing'],
  doctor: ['dashboard', 'dashboard_income', 'patients', 'patient_info',
    'patient_appointments', 'patient_records', 'patient_billing',
    'patient_create', 'patient_view', 'patient_edit',
    'appointments', 'appointments_calendar', 'waiting_list', 'medical_records',
    'doctor_finance', 'referral_letters', 'diagnosis_report', 'diagnosis_drugs'],
...
export default function Users() {
  ...
  useEffect(() => {
    getUsers().then((res) => { const d = res.data as any; setUsers(d.results || d) })
      .catch(() => toast.error('متأسفانه در دریافت اطلاعات کاربران خطایی رخ داد ')).finally(() => setLoading(false))
    getAvailablePermissions().then((res) => { const d = res.data as any; setAllPermissions(Array.isArray(d) ? d : d.results || []) }).catch(() => setAllPermissions([]))
  }, [])
  ...
  const openNew = () => {
    ...
    setForm({
      ...
      phone: '', role: 'reception', commission_percentage: 0, is_active: true,
      specialization: '', medical_council_number: '',
      page_permissions: [...(DEFAULT_ROLE_PERMISSIONS.reception || [])],
      two_factor_enabled: false,
    })
    setShowModal(true)
  }
  ...
  const openEdit = (u: User) => {
    ...
    setForm({
      ...
      page_permissions: u.page_permissions || [...(DEFAULT_ROLE_PERMISSIONS[u.role] || [])],
              ...
              <tbody>
                {users.map(u => (
                  <tr key={u.id}>
                    ...
                    <td>{u.medical_council_number ? toPersianDigits(u.medical_council_number) : '—'}</td>
                    <td dir="ltr">{u.phone || '—'}</td>
                    <td>{toPersianDigits(u.commission_percentage || 0)}%</td>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div><label className="label">نقش</label>
                    <select className="input-field" value={form.role} onChange={e => handleRoleChange(e.target.value)}>
                      <option value="reception">پذیرش</option><option value="doctor">پزشک / درمانگر</option><option value="rtms">کاربر ویژه</option><option value="admin">مدیر کلینیک</option>
                    </select>
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              {form.role === 'doctor' && (
                <>
                  ...
                  <div><label className="label">درصد سهم پزشک</label><input type="number" className="input-field" value={form.commission_percentage} onChange={e => setForm({ ...form, commission_percentage: Number(e.target.value) })} /></div>
                </>
              )}
