      ...
      render: (p) => (
        <div className="flex items-center justify-center gap-1">
          ...
          {hasRole('admin', 'support', 'super_support', 'reception') && (
            <button onClick={() => handleToggleRtms(p)} className={`action-btn ${p.visible_to_rtms ? 'text-brand-500' : ''}`} title={p.visible_to_rtms ? 'عدم نمایش به کاربران ویژه' : 'نمایش به کاربران ویژه'}><UsersIcon size={16} /></button>
          )}
              ...
              {patients.map((p: any, idx: number) => (
                <div key={p.id} className="table-card-item">
                  ...
                  <div className="table-card-actions">
                    ...
                    {hasRole('admin', 'support', 'super_support', 'reception') && <button onClick={() => handleToggleRtms(p)} className={`btn-ghost text-xs ${p.visible_to_rtms ? 'text-brand-500' : ''}`}><UsersIcon size={14} /> کاربر ویژه</button>}
