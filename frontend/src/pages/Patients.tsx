...
import { Plus, Search, Eye, Edit, Trash2, X, ChevronLeft, ChevronRight, Filter, User, Phone, FileText, CreditCard, Users as UsersIcon, Stethoscope, UserCircle, IdCard, CalendarDays, BookOpen, Briefcase, HeartPulse, AlertCircle, CheckCircle, XCircle } from 'lucide-react'
    ...
    },
    {
      ...
      render: (p) => (
        <div className="flex items-center justify-center gap-1">
          ...
          {p.created_by_role !== 'rtms' && hasRole('admin', 'reception') && (
              ...
              {patients.map((p: any, idx: number) => (
                <div key={p.id} className="table-card-item">
                  ...
                  <div className="table-card-actions">
                    ...
                    {p.created_by_role !== 'rtms' && hasRole('admin', 'reception') && <button onClick={() => handleToggleRtms(p)} className={`btn-ghost text-xs ${p.visible_to_rtms ? 'text-emerald-500' : ''}`}><UsersIcon size={14} /> کاربر ویژه</button>}
              ...
              <div className="flex items-center gap-1">
                ...
                {(() => {
                    ...
                    return pages.map((p, idx) =>
                      p === '...' ? (
                        <span key={`ellipsis-${idx}`} className="px-1 text-surface-400 select-none">...</span>
                      ) : (
                        <button key={p} onClick={() => loadPatients(search, p as number)}
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} size="xl" title={editing ? 'ویرایش بیمار' : 'بیمار جدید'}>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {renderFormSection('اطلاعات شخصی', <UserCircle size={16} />, [
            ...
            <SelectField key="job" label="شغل" name="job" placeholder="انتخاب کنید" options={[{ value: 'doctor', label: 'پزشک' }, { value: 'midwife', label: 'ماما' }, { value: 'engineer', label: 'مهندس' }, { value: 'nurse', label: 'پرستار' }, { value: 'teacher', label: 'معلم / استاد' }, { value: 'employee', label: 'کارمند' }, { value: 'worker', label: 'کارگر' }, { value: 'housewife', label: 'خانه دار' }, { value: 'freelance', label: 'آزاد' }]} />,
          ])}
