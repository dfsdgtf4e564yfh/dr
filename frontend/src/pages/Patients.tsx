    ...
    },
    {
      ...
      render: (p) => (
        <div className="flex items-center justify-center gap-1">
          ...
          {p.created_by_role !== 'rtms' && hasRole('admin', 'support', 'super_support', 'reception') && (
              ...
              {patients.map((p: any, idx: number) => (
                <div key={p.id} className="table-card-item">
                  ...
                  <div className="table-card-actions">
                    ...
                    {p.created_by_role !== 'rtms' && hasRole('admin', 'support', 'super_support', 'reception') && <button onClick={() => handleToggleRtms(p)} className={`btn-ghost text-xs ${p.visible_to_rtms ? 'text-emerald-500' : ''}`}><UsersIcon size={14} /> کاربر ویژه</button>}
