...
import { Plus, Search, Eye, Edit, Trash2, X, ChevronLeft, ChevronRight, Filter, User, Phone, FileText, CreditCard, Users as UsersIcon, Stethoscope, MapPin, UserCircle, IdCard, CalendarDays, BookOpen, Briefcase, HeartPulse, AlertCircle, CheckCircle, XCircle, Tag } from 'lucide-react'
import { getPatients, createPatient, updatePatient, deletePatient, searchPatients, getPatientTags, bulkAssignTags } from '../services/api'
import { useAuth } from '../contexts/AuthContext'
import { toPersianDigits, formatAge } from '../utils/jalali'
...
export default function Patients() {
  ...
  const loadPatients = async (q = '', p = 1, field = '') => {
    ...
    try {
      if (q && field && field !== 'all') {
        ...
        if (filterTagIds.length > 0) params.tag = filterTagIds[0]
  ...
  const openNew = () => {
    setEditing(null); setAge(''); setSelectedTags([])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editing) { await updatePatient(editing, payload); toast.success(`اطلاعات بیمار ${form.first_name} ${form.last_name} با موفقیت ویرایش شد`, { icon: <CheckCircle size={20} /> }) }
      else { const res = await createPatient(payload); patientId = res.data.id; toast.success(`بیمار ${form.first_name} ${form.last_name} با موفقیت به سیستم اضافه شد`, { icon: <CheckCircle size={20} /> }) }
      if (patientId) {
        await bulkAssignTags(patientId, selectedTags.map(t => t.id))
      }
      setShowModal(false); loadPatients(search)
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleSearch} className="flex gap-3 items-start max-w-xl">
        <div className="flex-1 flex gap-3">
          <div className="relative flex-1" ref={searchRef}>
            ...
            <input type="text"
              placeholder={searchField === 'national_id' ? 'جستجوی کد ملی...' : searchField === 'phone' ? 'جستجوی شماره تماس...' : searchField === 'file_number' ? 'جستجوی شماره پرونده...' : 'جستجو بر اساس نام، کد ملی، تلفن، شماره پرونده...'}
              ...
              onKeyDown={(e: React.KeyboardEvent<HTMLInputElement>) => {
                if (e.key === 'ArrowDown') { setSelectedSuggestion((prev: number) => Math.min(prev + 1, suggestions.length - 1)); e.preventDefault() }
                  ...
                  navigate(`/patients/${suggestions[selectedSuggestion].id}`); setShowSuggestions(false)
                }
              }}
              ...
              <div ref={suggestionRef} className="absolute top-full right-0 left-0 mt-1.5 bg-white border border-surface-200 rounded-2xl shadow-soft-lg z-50 max-h-60 overflow-y-auto">
                {suggestions.map((p: any, i: number) => (
                  <div key={p.id}
                    ...
                    onClick={() => { navigate(`/patients/${p.id}`); setShowSuggestions(false) }}
                  ...
                  >
                    ...
                    <div className="text-xs text-surface-400 shrink-0">{p.file_number && toPersianDigits(p.file_number)}</div>
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="card p-5 bg-surface-50/50">
          <div className="flex gap-4 items-end flex-wrap">
            ...
            <div className="min-w-[200px]">
              ...
              <select className="input-field" value={filterTagIds[0] || ''}
                onChange={(e: React.ChangeEvent<HTMLSelectElement>) => {
                  const val = e.target.value
                  setFilterTagIds(val ? [Number(val)] : [])
                }}>
                <option value="">همه</option>
                {allTags.map((t: any) => (
      ...
      {loading ? (
        <div className="panel card-iranian"><SkeletonTable rows={8} cols={8} /></div>
      ) : patients.length === 0 ? (
            ...
            <div className="table-wrap">
              <table>
                 <thead><tr>
                   ...
                   <th className="text-right">نام خانوادگی</th>
                   <th className="text-right">نام پدر</th>
                   <th className="text-right">کد ملی</th>
                    <th className="text-right">شماره تماس</th>
                    <th className="text-right">شماره پرونده</th>
                    <th className="text-right">برچسب‌ها</th>
                    <th className="text-center">عملیات‌ها</th>
                 </tr></thead>
                 <tbody>
                   {patients.map((p: any, idx: number) => (
                     <tr key={p.id}>
                       <td className="text-center font-bold text-surface-400">{toPersianDigits(idx + 1)}</td>
                       <td className="text-right font-medium">{p.first_name}</td>
                       <td className="text-right font-medium">{p.last_name}</td>
                       <td className="text-right">{p.father_name || '—'}</td>
                       <td className="text-right">{toPersianDigits(p.national_id)}</td>
                        <td className="text-right">{toPersianDigits(p.phone)}</td>
                        <td className="text-right font-bold text-brand-500">{toPersianDigits(p.file_number || p.id)}</td>
                        ...
                        <div className="flex items-center justify-center gap-1">
                          {hasPermission('patient_view') && (
                            <button onClick={() => navigate(`/patients/${p.id}`)} className="action-btn" title="مشاهده"><Eye size={16} /></button>
                          )}
              ...
              {patients.map((p: any, idx: number) => (
                <div key={p.id} className="table-card-item">
                  ...
                  <div className="table-card-row">
                    ...
                    <span className="table-card-value">{p.first_name} {p.last_name}</span>
                  </div>
                  <div className="table-card-row">
                  ...
                  <div className="table-card-row">
                    ...
                    <span className="table-card-value text-brand-500 font-bold">{toPersianDigits(p.file_number || p.id)}</span>
                  </div>
                  {(p.tags || []).length > 0 && (
                    <div className="table-card-row">
                      <span className="table-card-label">برچسب‌ها</span>
                  ...
                  <div className="table-card-actions">
                    {hasPermission('patient_view') && <button onClick={() => navigate(`/patients/${p.id}`)} className="btn-ghost text-xs"><Eye size={14} /> مشاهده</button>}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {renderFormSection('اطلاعات شخصی', <UserCircle size={16} />, [
            ...
            <div key="nid">
              ...
              {form.national_id.length >= 3 && !errors.national_id && getCityFromNationalId(form.national_id) && (
            ...
            <div key="job">
              ...
              <input className={`input-field ${errors.job ? 'error' : ''}`} name="job" value={form.job} onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateField('job', e.target.value)} />
              {errors.job && <p className="text-xs text-rose-500 mt-1.5 pr-1 font-medium">{errors.job}</p>}
            </div>,
          ])}

          <div className="bg-surface-50/50 rounded-2xl p-5 border border-surface-100">
            <h4 className="section-title">
              <span className="icon-wrap"><Tag size={16} /></span>
              برچسب‌ها
            </h4>
