...
import { toPersianDigits, formatAge, formatPhone, formatNationalId, formatGender } from '../utils/jalali'
...
export default function Patients() {
  ...
  const patientColumns: { key: string; label: string; tdClass?: string; render: (p: any, idx: number) => React.ReactNode }[] = [
    ...
    { key: 'phone', label: 'شماره تماس', render: (p) => toPersianDigits(p.phone) },
    {
      ...
      render: (p) => <span dir="ltr" className="inline-block">{toPersianDigits(p.file_number || p.id)}</span>,
    },
    {
