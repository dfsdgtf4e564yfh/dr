...
import { Plus, Edit2, Trash2, MessageSquare, DollarSign } from 'lucide-react'
import { getBillings, createBilling, updateBilling, deleteBilling, restoreBilling } from '../services/api'
              ...
              <tbody>
                {billings.map((b: any) => (
                  <tr key={b.id}>
                    ...
                    <td>{b.doctor_name}</td>
                    <td><span className={`text-xs px-2 py-0.5 rounded-full ${b.cost_type === 'visit' ? 'bg-blue-100 text-blue-700' : 'bg-teal-100 text-teal-700'}`}>{b.cost_type === 'visit' ? 'ویزیت' : 'خدمات'}</span></td>
                    <td>{formatMoney(b.total_amount)}</td>
                    <td>{formatMoney(b.paid_amount)}</td>
                    <td>{b.payment_method === 'cash' ? 'نقدی' : b.payment_method === 'card' ? 'کارت' : b.payment_method === 'insurance' ? 'بیمه' : 'کارت به کارت'}</td>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} size="md" title={editing ? 'ویرایش صورتحساب' : 'صورتحساب جدید'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div><label className="label">پزشک</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div><label className="label">درصد سهم پزشک</label><input type="number" className="input-field" value={form.doctor_commission_percentage} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setForm({ ...form, doctor_commission_percentage: e.target.value })} /></div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
