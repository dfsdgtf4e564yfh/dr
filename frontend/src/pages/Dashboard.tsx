...
import { getDashboardStats, getMonthlyIncome, getDoctorIncomePie, getPatientsTrend, getAlerts, getWorkYearInfo } from '../services/api'
...
export default function Dashboard() {
  ...
  const [workYear, setWorkYear] = useState<any>(null)
  ...
  const fetchData = useCallback(async (showLoading = true) => {
    ...
    try {
      const [s, mi, di, pt, al, wy] = await Promise.all([
        getDashboardStats(), getMonthlyIncome(), getDoctorIncomePie(),
        getPatientsTrend(30), getAlerts(), getWorkYearInfo(),
      ])
      setStats(s.data)
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
      ...
      <div className="grid grid-cols-12 gap-6">
        <motion.div variants={item} className="col-span-12 xl:col-span-4">
          <div className="card-gradient rounded-2xl p-6 h-full flex flex-col" style={{ background: 'linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%)' }}>
            ...
            <div className="mt-auto pt-4 flex gap-3">
              <button onClick={() => navigate('/notifications')} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold bg-white/15 text-white border border-white/20 hover:bg-white/25 transition-all backdrop-blur-sm">
                <Activity size={14} /> اعلانات
              </button>
              {canPatients && (
                <button onClick={() => navigate('/patients')} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold bg-white/15 text-white border border-white/20 hover:bg-white/25 transition-all backdrop-blur-sm">
        ...
        <motion.div variants={item} className={`col-span-12 xl:col-span-${statWidgets.length > 0 ? '8' : '0'}`}>
          {statWidgets.length > 0 && (
            <div className={`grid grid-cols-2 ${statWidgets.length > 2 ? 'lg:grid-cols-4' : `lg:grid-cols-${statWidgets.length}`} gap-4`}>
              {statWidgets.map((w: any, i: number) => (
                <div key={i} className="stat-card card-corner-ornament card-iranian card-corner-ornament">
      ...
      <motion.div variants={item} className="grid grid-cols-12 gap-6">
              ...
              <div className="flex flex-col xl:flex-row gap-6">
                ...
                <div className="xl:w-56 flex-shrink-0 flex flex-row xl:flex-col gap-3">
                  ...
                  <div className="flex items-center gap-3 p-4 rounded-xl bg-surface-50/70">
                    ...
                    <div className="min-w-0"><span className="text-xs text-surface-400 font-medium">بیماران (امسال)</span><h6 className="text-sm font-bold text-surface-800 mt-0.5">{formatMoney(workYear?.patients_count || 0)}</h6></div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className={`col-span-12 xl:col-span-${canIncome ? '4' : '12'}`}>
          <div className={`grid grid-cols-1 ${!canIncome ? 'md:grid-cols-2 xl:grid-cols-4' : ''} gap-4`}>
            {/* Doctor shares pie */}
            {canIncome && (
              <div className="card p-6">
                <h5 className="text-base font-bold text-surface-800 mb-5">سهم پزشکان</h5>
            ...
            {/* Alerts */}
            <div className="card p-6">
              <h5 className="text-sm font-bold text-surface-800 mb-4">هشدارها</h5>
      ...
      {/* Row 3: Trend + Mini cards */}
      <motion.div variants={item} className="grid grid-cols-12 gap-6">
        {/* Today's appointments */}
        <div className={`col-span-12 sm:col-span-6 xl:col-span-3 ${!canAppointments ? 'hidden' : ''}`}>
        ...
        <div className="col-span-12 sm:col-span-6 xl:col-span-3">
          {workYear ? (
            <div className="card-gradient h-full flex flex-col p-6" style={{ background: 'linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%)' }}>
              <h5 className="text-white/90 font-bold text-sm mb-5">سال کاری {toPersianDigits(workYear.jalali_year)}</h5>
              <div className="grid grid-cols-2 gap-3 flex-1">
                <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 flex flex-col items-center justify-center text-center">
                  <p className="text-2xl font-extrabold text-white leading-none mb-1">{formatMoney(workYear.patients_count || 0)}</p>
                  <p className="text-[11px] text-white/70">بیماران جدید</p>
                </div>
                <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 flex flex-col items-center justify-center text-center">
                  <p className="text-2xl font-extrabold text-white leading-none mb-1">{formatMoney(workYear.appointments_count || 0)}</p>
                  <p className="text-[11px] text-white/70">نوبت‌ها</p>
                </div>
                {canIncome && workYear.total_income !== undefined && (
                  <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 flex flex-col items-center justify-center text-center">
                    <p className="text-base font-extrabold text-white leading-none mb-1">{formatMoney(workYear.total_income)} تومان</p>
                    <p className="text-[11px] text-white/70">درآمد کل</p>
                  </div>
                )}
                {canIncome && workYear.total_paid !== undefined && (
                  <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 flex flex-col items-center justify-center text-center">
                    <p className="text-base font-extrabold text-white leading-none mb-1">{formatMoney(workYear.total_paid)} تومان</p>
                    <p className="text-[11px] text-white/70">دریافت شده</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="card p-6 h-full flex flex-col">
              <div className="flex-1 flex flex-col items-center justify-center">
                <Activity size={40} className="text-surface-300 mb-4" />
                <p className="text-sm text-surface-500 font-medium mb-4 text-center">ابزارهای مدیریت کلینیک</p>
                <div className="flex gap-2 flex-wrap justify-center">
                  {hasRole('admin') && (
                    <>
                      <button onClick={() => navigate('/users')} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold bg-brand-500 text-white hover:bg-brand-600 transition-all shadow-sm">
                        <UserPlus size={14} /> کاربران
                      </button>
                      <button onClick={() => navigate('/settings')} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold border-2 border-surface-200 text-surface-600 hover:border-brand-500 hover:text-brand-500 transition-all">
