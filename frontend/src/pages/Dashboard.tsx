            ...
            <div className="card !p-4">
              {workYear ? (
                <div className="flex flex-col">
                  <h5 className="text-sm font-bold text-surface-800 mb-4">سال کاری {toPersianDigits(workYear.jalali_year)}</h5>
                  <div className="grid grid-cols-2 gap-2">
                    <div className="bg-surface-50/70 rounded-xl p-3 flex flex-col items-center justify-center text-center">
                      <p className="text-lg font-extrabold text-surface-800 leading-none mb-1">{formatMoney(workYear.patients_count || 0)}</p>
                      <p className="text-[11px] text-surface-400">بیماران جدید</p>
                    </div>
                    <div className="bg-surface-50/70 rounded-xl p-3 flex flex-col items-center justify-center text-center">
                      <p className="text-lg font-extrabold text-surface-800 leading-none mb-1">{formatMoney(workYear.appointments_count || 0)}</p>
                      <p className="text-[11px] text-surface-400">نوبت‌ها</p>
                    </div>
                    {canIncome && workYear.total_income !== undefined && (
                      <div className="bg-surface-50/70 rounded-xl p-3 flex flex-col items-center justify-center text-center">
                        <p className="text-sm font-extrabold text-surface-800 leading-none mb-1">{formatMoney(workYear.total_income)} تومان</p>
                        <p className="text-[11px] text-surface-400">درآمد کل</p>
                      </div>
                    )}
                    {canIncome && workYear.total_paid !== undefined && (
                      <div className="bg-surface-50/70 rounded-xl p-3 flex flex-col items-center justify-center text-center">
                        <p className="text-sm font-extrabold text-surface-800 leading-none mb-1">{formatMoney(workYear.total_paid)} تومان</p>
                        <p className="text-[11px] text-surface-400">دریافت شده</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="flex flex-col items-center justify-center py-6">
                  <Activity size={32} className="text-surface-300 mb-3" />
                  <p className="text-xs text-surface-500 font-medium">ابزارهای مدیریت کلینیک</p>
                </div>
              )}
            </div>
          </div>
