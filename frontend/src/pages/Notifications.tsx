...
import {
  ...
  DollarSign, Activity, ArrowLeft, FileText, User, Zap
} from 'lucide-react'
                ...
                {data?.recent_logs?.length === 0 ? (
                  ...
                  data?.recent_logs?.map((l: any) => (
                    <tr key={l.id}>
                      ...
                      <td className="text-center text-slate-500">{l.model_name}</td>
                      <td className="text-center text-slate-400 text-[11px]">{toJalali(l.timestamp)}</td>
                    </tr>
                  ))
                )}
              </tbody>
