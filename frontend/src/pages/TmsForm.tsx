...
function generatePrintHtml(data: Record<string, any>, patientInfo: Record<string, any> | null) {
  ...
  <div class="page">
    ...
    <div class="patient-box">
      ...
      <div class="item"><span class="lbl">شغل:</span><span class="val">${({ doctor: 'پزشک', midwife: 'ماما', engineer: 'مهندس', nurse: 'پرستار', teacher: 'معلم / استاد', employee: 'کارمند', worker: 'کارگر', housewife: 'خانه دار', freelance: 'آزاد' })[p.job] || p.job || '—'}</span></div>
  ...
  <div class="page">
    ...
    <div class="section">
      <div class="section-title">نمرات</div>
      <div class="section-body">
        <div style="display:flex;flex-wrap:wrap;gap:8px;">
          <div style="flex:1;min-width:60px;text-align:center;padding:4px 6px;background:#f8fafc;border:1px solid #e2e8f0;"><div style="font-weight:700;color:#1e3a5f;font-size:8px;">C</div><div style="font-size:10px;font-weight:700;margin-top:2px;">${d.score_c || '—'}</div></div>
          <div style="flex:1;min-width:60px;text-align:center;padding:4px 6px;background:#f8fafc;border:1px solid #e2e8f0;"><div style="font-weight:700;color:#1e3a5f;font-size:8px;">NI</div><div style="font-size:10px;font-weight:700;margin-top:2px;">${d.score_ni || '—'}</div></div>
          <div style="flex:1;min-width:60px;text-align:center;padding:4px 6px;background:#f8fafc;border:1px solid #e2e8f0;"><div style="font-weight:700;color:#1e3a5f;font-size:8px;">TT</div><div style="font-size:10px;font-weight:700;margin-top:2px;">${d.score_tt || '—'}</div></div>
          <div style="flex:1;min-width:60px;text-align:center;padding:4px 6px;background:#f8fafc;border:1px solid #e2e8f0;"><div style="font-weight:700;color:#1e3a5f;font-size:8px;">X</div><div style="font-size:10px;font-weight:700;margin-top:2px;">${d.score_x || '—'}</div></div>
          <div style="flex:1;min-width:60px;text-align:center;padding:4px 6px;background:#f8fafc;border:1px solid #e2e8f0;"><div style="font-weight:700;color:#1e3a5f;font-size:8px;">Y</div><div style="font-size:10px;font-weight:700;margin-top:2px;">${d.score_y || '—'}</div></div>
          <div style="flex:1;min-width:60px;text-align:center;padding:4px 6px;background:#f8fafc;border:1px solid #e2e8f0;"><div style="font-weight:700;color:#1e3a5f;font-size:8px;">Mt80</div><div style="font-size:10px;font-weight:700;margin-top:2px;">${d.score_mt80 || '—'}</div></div>
          <div style="flex:1;min-width:60px;text-align:center;padding:4px 6px;background:#f8fafc;border:1px solid #e2e8f0;"><div style="font-weight:700;color:#1e3a5f;font-size:8px;">Mt90</div><div style="font-size:10px;font-weight:700;margin-top:2px;">${d.score_mt90 || '—'}</div></div>
          <div style="flex:1;min-width:60px;text-align:center;padding:4px 6px;background:#f8fafc;border:1px solid #e2e8f0;"><div style="font-weight:700;color:#1e3a5f;font-size:8px;">Mt100</div><div style="font-size:10px;font-weight:700;margin-top:2px;">${d.score_mt100 || '—'}</div></div>
          <div style="flex:1;min-width:60px;text-align:center;padding:4px 6px;background:#f8fafc;border:1px solid #e2e8f0;"><div style="font-weight:700;color:#1e3a5f;font-size:8px;">Mt120</div><div style="font-size:10px;font-weight:700;margin-top:2px;">${d.score_mt120 || '—'}</div></div>
          <div style="flex:1;min-width:60px;text-align:center;padding:4px 6px;background:#f8fafc;border:1px solid #e2e8f0;"><div style="font-weight:700;color:#1e3a5f;font-size:8px;">Mt130</div><div style="font-size:10px;font-weight:700;margin-top:2px;">${d.score_mt130 || '—'}</div></div>
        </div>
      </div>
    </div>
...
export default function TmsForms() {
  ...
  const openEdit = async (id: number) => {
    try {
      ...
      setFormData({
        ...
        score_c: form.score_c?.toString() || '',
        score_ni: form.score_ni?.toString() || '',
        score_tt: form.score_tt?.toString() || '',
        score_x: form.score_x?.toString() || '',
        score_y: form.score_y?.toString() || '',
        score_mt80: form.score_mt80?.toString() || '',
        score_mt90: form.score_mt90?.toString() || '',
        score_mt100: form.score_mt100?.toString() || '',
        score_mt120: form.score_mt120?.toString() || '',
        score_mt130: form.score_mt130?.toString() || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    const scoreKeys = ['score_c', 'score_ni', 'score_tt', 'score_x', 'score_y',
      'score_mt80', 'score_mt90', 'score_mt100', 'score_mt120', 'score_mt130']
    const scores: Record<string, number | undefined> = {}
    for (const k of scoreKeys) {
      const v = (formData as any)[k]
      scores[k] = v !== '' ? Number(v) : undefined
    }
    ...
    return (
      <div className="max-w-5xl mx-auto space-y-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {patientInfo && (
                <div className="mt-3 p-3 bg-slate-50 rounded-lg grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                  ...
                  <div className="col-span-2"><span className="text-gray-500">شغل:</span> <span className="font-medium">{{
                    ...
                    teacher: 'معلم / استاد', employee: 'کارمند', worker: 'کارگر', housewife: 'خانه دار', freelance: 'آزاد'
          ...
          <div className="card">
            <div className="panel-header panel-header-iranian"><h3>نمرات</h3></div>
            <div className="panel-body">
              <div className="grid grid-cols-3 md:grid-cols-5 gap-4">
                <div><label className="label">C</label><input className="input-field" type="number" step="any" value={formData.score_c} onChange={e => updateField('score_c', e.target.value)} /></div>
                <div><label className="label">NI</label><input className="input-field" type="number" step="any" value={formData.score_ni} onChange={e => updateField('score_ni', e.target.value)} /></div>
                <div><label className="label">TT</label><input className="input-field" type="number" step="any" value={formData.score_tt} onChange={e => updateField('score_tt', e.target.value)} /></div>
                <div><label className="label">X</label><input className="input-field" type="number" step="any" value={formData.score_x} onChange={e => updateField('score_x', e.target.value)} /></div>
                <div><label className="label">Y</label><input className="input-field" type="number" step="any" value={formData.score_y} onChange={e => updateField('score_y', e.target.value)} /></div>
                <div><label className="label">Mt80</label><input className="input-field" type="number" step="any" value={formData.score_mt80} onChange={e => updateField('score_mt80', e.target.value)} /></div>
                <div><label className="label">Mt90</label><input className="input-field" type="number" step="any" value={formData.score_mt90} onChange={e => updateField('score_mt90', e.target.value)} /></div>
                <div><label className="label">Mt100</label><input className="input-field" type="number" step="any" value={formData.score_mt100} onChange={e => updateField('score_mt100', e.target.value)} /></div>
                <div><label className="label">Mt120</label><input className="input-field" type="number" step="any" value={formData.score_mt120} onChange={e => updateField('score_mt120', e.target.value)} /></div>
                <div><label className="label">Mt130</label><input className="input-field" type="number" step="any" value={formData.score_mt130} onChange={e => updateField('score_mt130', e.target.value)} /></div>
              </div>
            </div>
          </div>
