...
import { toJalali, toPersianDigits, formatAge } from '../utils/jalali'
...
export default function TmsForms() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err?.response?.data?.detail || err?.response?.data?.[0] || 'متأسفانه در ذخیره فرم TMS خطایی رخ داد '
      toast.error(typeof msg === 'string' ? msg : 'متأسفانه در ذخیره فرم TMS خطایی رخ داد ')
    } finally {
      setSaving(false)
    }
  }
