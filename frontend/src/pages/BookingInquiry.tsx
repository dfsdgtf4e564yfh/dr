...
import { toEnglishDigits, toJalaliNumeric, nowJalali } from '../utils/jalali'
...
export default function BookingInquiry() {
  ...
  useEffect(() => {
    ...
    const stored = localStorage.getItem('inquiry_' + nationalId)
    if (stored) {
      setData(JSON.parse(stored))
      setLoading(false)
      return
    }
    myAppointments(nationalId)
      .then(({ data }) => { setData(data); localStorage.setItem('inquiry_' + nationalId, JSON.stringify(data)) })
      ...
      <div className="py-16 px-4">
        <div className="max-w-2xl mx-auto">
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-gray-400">کد ملی: {nationalId}</p>
          </div>
                ...
                <div className="bg-white rounded-3xl shadow-xl shadow-gray-200/50 p-5 border border-gray-100">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      {data.patient.file_number && <p className="text-xs text-gray-400">شماره پرونده: {data.patient.file_number}</p>}
                    </div>
                  </div>
                </div>
              )}
