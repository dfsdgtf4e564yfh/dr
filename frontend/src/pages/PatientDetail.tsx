...
import { toJalali, toPersianDigits, smartPersianDigits, formatMoney, formatAge, escapeHtml, mediaUrl } from '../utils/jalali'
...
<div class="paper">
    ...
    <div class="header-inner">
      ...
      <div class="header-left">
        <div class="badge-box">
          ...
          <span class="val">${smartPersianDigits(p.file_number || '—')}</span>
        </div>
      </div>
    </div>
  </div>
        ...
        {editing ? (
          <div className="card p-6">
            ...
            <form onSubmit={handleEdit} className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div><JalaliDateInput label="تاریخ تولد" value={birthDate} onChange={v => setBirthDate(v)} /></div>
            ...
            <div className="card p-5">
              <div className="flex items-center gap-4 mb-5 pb-4 border-b border-surface-100">
                ...
                <div className="min-w-0">
                  ...
                  {patient.file_number && <p className="text-sm text-brand-500 font-bold">شماره پرونده: {smartPersianDigits(patient.file_number)}{patient.old_file_number ? <span className="text-surface-400 font-normal mr-3">قدیمی: {smartPersianDigits(patient.old_file_number)}</span> : ''}</p>}
                </div>
              </div>
  ...
  teacher: 'معلم / استاد', employee: 'کارمند', worker: 'کارگر', housewife: 'خانه دار', freelance: 'آزاد'
                ...
                {patient.old_file_number && <div className="flex items-center gap-2"><span className="text-surface-400 font-medium shrink-0">شماره پرونده قدیمی:</span> <span className="truncate">{smartPersianDigits(patient.old_file_number)}</span></div>}
                  ...
                  {billings.length > 0 && (
                    <div className="grid grid-cols-3 gap-3 mb-5">
                      {(() => {
                        ...
                        const totalPaid = billings.reduce((s, b) => s + Number(b.paid_amount || 0), 0)
              ...
              {activeTab === 'info' && (
                <div className="space-y-5">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-sm">
                    <div className="flex items-center gap-2"><span className="text-surface-400 font-medium min-w-[90px]">شماره پرونده:</span><span className="font-semibold text-surface-700">{patient.file_number ? smartPersianDigits(patient.file_number) : '—'}</span></div>
                    <div className="flex items-center gap-2"><span className="text-surface-400 font-medium min-w-[90px]">پرونده قدیمی:</span><span className="font-semibold text-surface-700">{patient.old_file_number ? smartPersianDigits(patient.old_file_number) : '—'}</span></div>
