...
import { ArrowRight, Phone, Calendar as CalIcon, MapPin, FileText, Activity, DollarSign, Printer, Image, Edit2, CheckCircle, XCircle, Tag } from 'lucide-react'
import { getPatient, updatePatient, getAppointments, getMedicalRecords, getBillings, getClinicSettings, bulkAssignTags, getPatientIdentity, createPatientIdentity, updatePatientIdentity, verifyPatientIdentity, registerFingerprint } from '../services/api'
...
export default function PatientDetail() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getPatientIdentity(patientId),
    ]).then(([p, a, r, b, id]: any[]) => {
      setPatient(p.data)
      setBirthDate(p.data.birth_date || '')
      setSelectedTags(p.data.tags || [])
      ...
      const identityData = Array.isArray(id.data) ? id.data[0] : id.data
  ...
  const allTabs = [
    ...
    { id: 'identity', label: 'هویت', permission: 'patient_identity_view' },
          ...
          <>
            ...
            <div className="card p-4 flex items-center justify-between">
              <div className="flex items-center gap-2 flex-1 flex-wrap">
                ...
                {!editingTags ? (
                  <>
                    {(patient.tags || []).length > 0 ? patient.tags.map((t: any) => (
                      <span key={t.id}
                        className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold"
                        style={{ backgroundColor: t.color + '20', color: t.color, border: `1px solid ${t.color}40` }}>
                        {t.name}
                      </span>
                    )) : <span className="text-sm text-surface-400">برچسبی انتخاب نشده</span>}
                  </>
                ) : (
              ...
              {!editingTags ? (
                <button onClick={() => setEditingTags(true)} className="text-xs text-brand-500 hover:text-brand-600 font-medium whitespace-nowrap">
                  ویرایش برچسب‌ها
                </button>
              ) : (
                <div className="flex gap-2">
                  <button onClick={async () => {
                    try {
                      await bulkAssignTags(Number(id), selectedTags.map((t: any) => t.id))
                      const { data } = await getPatient(Number(id))
                      setPatient(data)
                      setEditingTags(false)
                      toast.success('برچسب‌ها با موفقیت به‌روز شدند', { icon: <CheckCircle size={20} /> })
                    } catch { toast.error('خطا در ذخیره برچسب‌ها', { icon: <XCircle size={20} /> }) }
                  }} className="text-xs text-success-500 hover:text-success-600 font-medium">ذخیره</button>
                  <button onClick={() => { setEditingTags(false); setSelectedTags(patient.tags || []) }} className="text-xs text-rose-500 hover:text-rose-600 font-medium">لغو</button>
                </div>
              )}
            </div>
