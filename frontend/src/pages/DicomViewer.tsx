...
const API_BASE = (window as any).APP_CONFIG?.API_BASE_URL || '/api'
          ...
          {selectedFile ? (
            <div className="card p-0 overflow-hidden">
              <div className="flex items-center justify-between p-3 border-b border-surface-100 bg-surface-50/50">
                ...
                <div className="flex items-center gap-2">
                  <a href={`${API_BASE}/medical-records/dicom-files/${selectedFile.id}/download/?token=${sessionStorage.getItem('access_token') || ''}`}
              ...
              <div className="flex relative" style={{ minHeight: '60vh' }}>
                <div className="flex-1 flex items-center justify-center bg-surface-900/5 overflow-hidden p-4">
                  <img src={`${API_BASE}/medical-records/dicom-files/${selectedFile.id}/view_image/?token=${sessionStorage.getItem('access_token') || ''}`}
                    alt="DICOM"
        ...
        <div className="space-y-4">
          <div>
            ...
            <input type="text" className="input-field" placeholder="جستجوی بیمار..." value={uploadPatient}
              onChange={e => { setUploadPatient(e.target.value); loadPatients(e.target.value) }} />
            {patientsLoading && <p className="text-xs text-surface-400 mt-1">در حال بارگذاری...</p>}
            {patients.length > 0 && (
              <div className="mt-1 border border-surface-200 rounded-xl overflow-hidden max-h-48 overflow-y-auto">
                {patients.map(p => (
          ...
          <div>
            ...
            <input type="file" accept=".dcm,.DCM,.jpg,.jpeg,.png,.gif,.bmp,.webp" onChange={e => setUploadFile(e.target.files?.[0] || null)}
