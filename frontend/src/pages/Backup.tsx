...
import { Download, Trash2, RefreshCw, Database, FileDown, Upload, FileSpreadsheet, Mail, CheckCircle, XCircle, Save, Settings } from 'lucide-react'
import { getBackups, createBackup, downloadBackup, deleteBackup, restoreBackup, exportPatientsCsv, exportAppointmentsCsv, exportMedicalRecordsCsv, exportBillingsCsv, importPatientsExcel, getEmailConfig, updateEmailConfig, testEmailConnection, sendBackupViaEmail } from '../services/api'
import { toJalali, toPersianDigits } from '../utils/jalali'
import ProgressBar from '../components/ProgressBar'
import type { BackupItem, EmailConfig } from '../types'
...
export default function Backup() {
  ...
  useEffect(() => { load(); loadEmailConfig() }, [])
          ...
          {loading ? (
            ...
            <div className="card overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {backups.length === 0 ? (
                    <tr><td colSpan={4} className="text-center py-10 text-gray-400">هیچ فایل پشتیبان یافت نشد</td></tr>
                  ) : backups.map((b, idx) => (
