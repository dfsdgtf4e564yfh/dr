...
import { toJalali, toPersianDigits, toEnglishDigits, formatMoney, escapeHtml } from '../utils/jalali'
...
export default function Appointments() {
  ...
  const printReceipt = (a: any, settings: any) => {
    ...
    const clinicPhone = (settings?.phone || '').replace(/-/g, '')
    const clinicPhone2 = (settings?.phone2 || '').replace(/-/g, '')
    ...
    <body><div class="receipt">
      ...
      <div class="info-box">
        ...
        <div class="row"><span class="label">پزشک</span><span class="value">${h(a.doctor_name)}</span></div>
      ...
      <div class="footer">
        ...
        <p>شماره تماس: ${clinicPhone}${clinicPhone2 ? ` | ${clinicPhone2}` : ''}</p>
      </div>
    </div></body></html>`)
    win!.document.close()
  }
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="panel-body">
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
            <div className="relative">
              ...
              <input type="text" className="input-field text-sm pr-9" placeholder="جستجوی نام یا کد ملی..." value={filters.search}
      ...
      {loading ? (
        <div className="panel"><SkeletonTable rows={6} cols={9} /></div>
      ) : groupedData.length === 0 ? (
        <EmptyState icon={CalendarIcon} title="نوبتی یافت نشد" description="نوبتی با فیلترهای فعلی وجود ندارد. برای ثبت نوبت جدید از دکمه بالا استفاده کنید." action={<Button onClick={openNew} icon={Plus}>نوبت جدید</Button>} />
      ) : (
        <>
                ...
                {groupedData.flatMap((group: any) => {
                  ...
                  const rows: any[] = [
                    <tr key={group.patient_id} className="cursor-pointer hover:bg-slate-50 transition-colors border-b border-slate-100"
                      ...
                      <td className="text-center text-slate-600 text-sm whitespace-nowrap">{group.patient_file_number ? toPersianDigits(group.patient_file_number) : '—'}</td>
